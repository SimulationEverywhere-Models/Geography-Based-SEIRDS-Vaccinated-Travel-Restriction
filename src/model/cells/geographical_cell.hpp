...
class geographical_cell : public cell<T, string, sevirds, vicinity>
{
    public:
            ...
            cell<T, string, sevirds, vicinity>(cell_id, neighborhood, initial_state, delay_id)
        {
            ...
            is_threaded             = config.is_threaded;
        ...
        sevirds local_computation() const override
        {
            ...
            double new_s, vac_total, nvac_total;
            ...
            for (unsigned int age_segment_index = 0; age_segment_index < age_segments; ++age_segment_index)
            {
                ...
                */
                ...
                double& res_susceptible         = res.susceptible.at(age_segment_index);
                ...
                if (is_threaded)
                {
                    thread th_nvac(&geographical_cell::compute_not_vaccinated, age_segment_index, res_susceptible, ref(res_fatalities), ref(res_exposed),
                                    ref(res_infected), ref(res_recovered), ref(nvac_total), exposed_phases, infected_phases, recovered_phases, this);
                ...
                {
                    if (is_vaccination)
                        compute_vaccinated(age_segment_index, res_susceptible, res_vaccinated1, res_vaccinated2, vac_total, vac1_phases, vac2_phases, this);
                    compute_not_vaccinated(age_segment_index, res_susceptible, res_fatalities, res_exposed, res_infected, res_recovered, nvac_total,
                                            exposed_phases, infected_phases, recovered_phases, this);
                }

                new_s = 1 - vac_total - nvac_total;

                if (new_s > -0.001 && new_s < 0) new_s = 0; // double precision issues
                assert(new_s >= 0);

                res_susceptible = new_s;
            } //for(age_groups)
        ...
        double new_vaccinated1(double res_susceptible, double vac1_rate) const { return vac1_rate * res_susceptible; }

        double new_vaccinated2(vector<double>& vac1, vector<double> const& vac2_rates) const
        {
            double vac2 = 0;
            ...
            unsigned interval = vac1.size() - state.current_state.min_interval_doses;
            ...
            for (unsigned int phase = 0; phase < interval; ++phase)
                vac2 += vac2_rates.at(phase) * vac1.at(state.current_state.min_interval_doses + phase);

            return vac2;
        }

        double new_exposed(unsigned int age_segment_index, double res_susceptible) const
        {
            double expos = 0;
            sevirds cstate = state.current_state;
            ...
            + (1 - cstate.disobedient) * movement_correction_factor(self_vicinity.correction_factors, 
                                                        ...
                                                        cstate.hysteresis_factors.at(cell_id));
            ...
            for(auto neighbor: neighbors)
            {
                ...
                neighbor_correction = nstate.disobedient + (1 - nstate.disobedient) * movement_correction_factor(v.correction_factors,
                                                                                                        ...
                                                                                                        cstate.hysteresis_factors.at(neighbor));
        ...
        double new_infections(const vector<double>& res_incubation_rates, vector<double> const& res_exposed) const
        {
            double inf = 0;
        ...
        vector<double> new_recoveries(unsigned int age_segment_index, vector<double> &fatalities,
                                        vector<double>& res_infected, unsigned int infected_phases) const
        {
        ...
        vector<double> new_fatalities(unsigned int age_segment_index, vector<double>& res_infected,
                                        unsigned int infected_phases) const
        {
            const sevirds& current_state = state.current_state;
        ...
        static void compute_vaccinated(int age_segment_index, double res_susceptible, vector<double>& res_vaccinated1, vector<double>& res_vaccinated2, double& new_s, unsigned int vac1_phases, unsigned int vac2_phases, const geographical_cell* pointer)
        {
            const double& curr_vac1_rates           = pointer->vac1_rates.at(age_segment_index);
            const vector<double>& curr_vac2_rates   = pointer->vac2_rates.at(age_segment_index);
            double curr_vac1, curr_vac2;

            // Calculate the number of new vaccinated dose 1
            double new_vac1 = pointer->precision_correction(pointer->new_vaccinated1(res_susceptible, curr_vac1_rates));

            // Calculate the number of new vaccinated dose 2
            double new_vac2 = pointer->precision_correction(pointer->new_vaccinated2(res_vaccinated1, curr_vac2_rates));
            ...
            for (unsigned int i = vac1_phases; i > 0; --i)
            {
                ...
                curr_vac1 = pointer->precision_correction(res_vaccinated1.at(i - 1) - pointer->vaccination2_rate(curr_vac2_rates, i - 1) * res_vaccinated1.at(i - 1));
        ...
        static void compute_not_vaccinated(int age_segment_index, double res_susceptible, double& res_fatalities, vector<double>& res_exposed,
                                            vector<double>& res_infected, vector<double>& res_recovered, double& new_s, unsigned int exposed_phases,\
                                            unsigned int infected_phases, unsigned int recovered_phases, const geographical_cell* pointer)
        {
            // Get these ahead of time for performance
            const vector<double> &res_incubation_rates  = pointer->incubation_rates.at(age_segment_index);
            double curr_expos, curr_inf;

            // Calculate the vector of fatalities entered from each infection day
            vector<double> fatalities = pointer->new_fatalities(age_segment_index, res_infected, infected_phases);

            // Calculate the vector of new recoveries entering from each infection day 1:num_infection_phases,
            vector<double> recovered = pointer->new_recoveries(age_segment_index, fatalities, res_infected, infected_phases);

            // <EXPOSED>
                // Calculate the total number of new exposed entering exposed(0)
                double new_e = pointer->precision_correction(pointer->new_exposed(age_segment_index, res_susceptible));
                ...
                for (unsigned int i = exposed_phases - 1; i > 0; --i)
                {
                    ...
                    curr_expos = pointer->precision_correction(res_exposed.at(i - 1) * (1 - res_incubation_rates.at(i - 1)));
            ...
            // <INFECTED>
                ...
                double new_i = pointer->precision_correction(pointer->new_infections(res_incubation_rates, res_exposed));
                ...
                for (unsigned int i = infected_phases - 1; i > 0; --i)
                {
                    ...
                    curr_inf = pointer->precision_correction(curr_inf);
        ...
        double vaccination2_rate(const vector<double>& curr_vac2_rates, int day) const
        {
            if (day < state.current_state.min_interval_doses) return 0;
            return curr_vac2_rates.at(day - state.current_state.min_interval_doses);
        }
